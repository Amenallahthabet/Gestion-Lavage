...
<div class="container">
    <div class="form-container">
        <h2 class="form-title">Modifier un Service</h2>
        <form action="{{ route('service.update', $service->id) }}" method="POST">
            ...
            <div class="form-group">
                <label for="tempservice">Temps de Service</label>
                    <select id="tempservice" name="tempservice" class="form-control">
                        <option value="">Sélectionnez un service</option>
                        @foreach($tempsOptions  as $option)
                            <option value="{{ $option }}" class="a" value="{{ $option }}" {{ $service->tempservice == $option ? 'selected' : '' }}>
                                {{ $option }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group">
                <label for="prixservice">Temp de Service</label>
                <input type="number" class="form-control" id="prixservice" name="prixservice" value="{{ $service->prixservice }}"  required>
            </div>
